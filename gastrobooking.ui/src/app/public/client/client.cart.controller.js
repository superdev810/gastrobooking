...
(function () {
    ...
    function ClientCartController($state, $rootScope,$scope,$timeout,ClientCartService,ClientService, uiDatetimePickerConfig, $translate, $stateParams, moment, appConstant, RestaurantDetailService, $geolocation) {
        ...
        $scope.$watch('vm.client', function (newValue) {
            if(newValue && vm.order) {
                if(!vm.order.delivery_address) {
                    vm.order.delivery_address = newValue.address;
                }
                if(!vm.order.delivery_phone) {
                    vm.order.delivery_phone = newValue.phone;
                }
            }
        });

        $scope.$watch('vm.order', function (newValue) {
            if(newValue && vm.client) {
                if(!newValue.delivery_address) {
                    newValue.delivery_address = vm.client.address;
                }
                if(!newValue.delivery_phone) {
                    newValue.delivery_phone = vm.client.phone;
                }
            }
        });
        ...
        function getCurrentClient() {
            ClientService.getCurrentClient().then(function (response) {
                ...
                vm.currentPosition = vm.currentPosition ? vm.currentPosition : vm.client.location;
                // vm.order.delivery_address = vm.order.delivery_address ? vm.order.delivery_address : vm.client.address;
                // vm.order.delivery_phone = vm.order.delivery_phone ? vm.order.delivery_phone : vm.client.phone;
        ...
        function getOrder(orderId) {
            ...
            ClientService.getOrder(orderId).then(function (response) {
                ...
                if (vm.deliveryAvailable && (vm.order.delivery_address !== null || vm.order.delivery_phone !== null)) {
                    ...
                    vm.currentPosition = vm.client.location;
                }                
        ...
        function locale() {
            ...
            locationpicker.locationpicker({
                location: {latitude: vm.order.delivery_latitude || vm.client.latitude, longitude: vm.order.delivery_longitude || vm.client.longitude},
                radius: 0,
                zoom: 15,
